    ...
    IIdentityDbContext
{
    ...
    // Agrega esta l韓ea para la nueva entidad
    ...
    // Inyecci贸n de ICurrentUser (null en design-time)
    ...
    protected override void OnModelCreating(ModelBuilder builder)
    {
        ...
        builder.Entity<Calificacion>(b =>
        {
            ...
            b.Property(x => x.Comentario).HasMaxLength(512); // Siempre es bueno poner un l铆mite

            b.Property(x => x.DestinoId).IsRequired().HasColumnType("uniqueidentifier");
            // 4. Configura la relaci贸n con el Destino
            ...
            // 5. Configura la relaci贸n con el Usuario (de IUserOwned)
            b.HasOne<IdentityUser>().WithMany()
                .HasForeignKey(x => x.UserId)
                .IsRequired()
                .OnDelete(DeleteBehavior.NoAction); // No borrar usuarios si se borra una calificaci贸n
        });
        // Configuraci髇 opcional de la tabla MetricaApi
        ...
        builder.Entity<Evento>(b =>
        {
            ...
            // Configuraci髇 del 韓dice 鷑ico compuesto para evitar duplicados por DestinoTuristicoId + IdExterno
        ...
        // Obtener el ID del usuario actual.Si es nulo(ej.fuera de una petici贸n http), usa Guid.Empty.
        ...
        foreach (var entityType in builder.Model.GetEntityTypes())
        {
            if (typeof(IUserOwned).IsAssignableFrom(entityType.ClrType))
            {
                ...
                var method = typeof(WayFinderDbContext)
                    ...
                    .MakeGenericMethod(entityType.ClrType); // Hacer el m茅todo gen茅rico para el tipo actual

                method.Invoke(this, new object[] { builder }); // Ejecutar el m茅todo con la configuraci贸n de EF Core
            }
        }
    ...
    private void ApplyUserOwnedFilter<TEntity>(ModelBuilder builder) where TEntity : class, IUserOwned
    {
        builder.Entity<TEntity>().HasQueryFilter(e =>
            _currentUser != null && _currentUser.IsAuthenticated // 1. Chequeo de seguridad
                ? e.UserId == _currentUser.GetId() // 2.Filtro si est谩 autenticado
                : false // 3. Si no est谩, no devuelve filas (seguridad estricta)
        );
